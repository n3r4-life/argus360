...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  // Reader Mode: auto-inject into new/navigated tabs when globally enabled or domain is on learned list
  if (changeInfo.status === "complete" && tab?.url) {
    const skipRe = /^(about:|moz-extension:|chrome:|chrome-extension:|file:)/;
    if (!skipRe.test(tab.url)) {
      browser.storage.local.get({ readerModeGlobal: false, readerSites: [] }).then(({ readerModeGlobal, readerSites }) => {
        let shouldInject = readerModeGlobal;
        if (!shouldInject && readerSites.length) {
          let hostname = "";
          try { hostname = new URL(tab.url).hostname.replace(/^www\./, ""); } catch {}
          shouldInject = readerSites.some(d => hostname === d || hostname.endsWith("." + d));
        }
        if (!shouldInject) return;
        // mountFn is stringified & injected into the tab — must be fully self-contained.
        // Polls every 400ms (max 25×) so SPAs like Twitter/X get content after React renders.
        const mountFn = function () {
          if (document.getElementById("__argus_reader_overlay__")) return;
          const CANDIDATES = ["article","[role='article']","main","[role='main']",".post-content",".entry-content",".article-body",".article-content",".story-body",".story-content",".content-body",".page-content","#article-body","#article-content","#post-content","#main-content",".post","#post",".entry","#entry","#content",".content","#main","main"];
          const STRIP = ["script","style","noscript","iframe","form","nav","header","footer","aside","button","[class*='ad-']","[class*='-ad']","[id*='cookie']","[class*='cookie']","[class*='subscribe']","[class*='newsletter']","[class*='popup']","[class*='modal']","[class*='sidebar']","[class*='related']","[class*='recommended']","[class*='share']","[class*='social']"];
          function findBest() {
            let best = null, bestLen = 0;
            for (const sel of CANDIDATES) {
              try { const el = document.querySelector(sel); if (!el) continue; const txt = el.innerText || ""; if (txt.length > bestLen) { best = el; bestLen = txt.length; } } catch {}
            }
            return { best, bestLen };
          }
          function buildOverlay(best) {
            ...
            for (const s of STRIP) { try { clone.querySelectorAll(s).forEach(e => e.remove()); } catch {} }
          ...
          function tryMount(attempt) {
            ...
            if (bestLen >= 500) { buildOverlay(best); }
