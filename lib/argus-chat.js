...
const ArgusChat = (() => {
  ...
  function createActionBar(rawMarkdown, questionText) {
    ...
    if (typeof TextIt !== "undefined") {
      ...
      textItBtn.addEventListener("click", () => TextIt.open(rawMarkdown));
      // Hide if XMPP not configured
      TextIt.isConfigured().then(ok => { if (!ok) textItBtn.classList.add("hidden"); });
  ...
  function createDOM(container) {
    ...
    // ACC (Add Collection Context) row — only if AssetLibrary is available
    const accRow = document.createElement("div");
    accRow.className = "argus-chat-acc-row";
    accRow.style.cssText = "display:none;padding:4px 8px;border-top:1px solid var(--border);";

    if (typeof AssetLibrary !== 'undefined') {
      const accBtn = document.createElement("button");
      accBtn.className = "pill-chip";
      accBtn.textContent = "ACC";
      accBtn.title = "Add Collection Context — share asset library items with AI";
      accBtn.style.cssText = "font-size:9px;";

      const accPicker = document.createElement("div");
      accPicker.className = "argus-chat-acc-picker";
      accPicker.style.cssText = "display:none;max-height:150px;overflow-y:auto;margin-top:4px;flex-direction:column;gap:2px;";

      const accConfirm = document.createElement("button");
      accConfirm.className = "pill-chip";
      accConfirm.textContent = "Add to Context";
      accConfirm.style.cssText = "font-size:9px;display:none;margin-top:4px;";

      accBtn.addEventListener("click", async () => {
        const isOpen = accPicker.style.display !== "none";
        if (isOpen) {
          accPicker.style.display = "none";
          accConfirm.style.display = "none";
          return;
        }

        // Populate picker with asset library items
        const items = await AssetLibrary.list();
        accPicker.innerHTML = "";
        if (!items.length) {
          accPicker.innerHTML = '<div style="font-size:10px;color:var(--text-muted);padding:6px;">No assets in collection.</div>';
          accPicker.style.display = "flex";
          return;
        }

        // Select all checkbox
        const selAllRow = document.createElement("label");
        selAllRow.style.cssText = "display:flex;align-items:center;gap:6px;padding:3px 4px;font-size:10px;color:var(--text-secondary);cursor:pointer;font-weight:700;";
        const selAllCb = document.createElement("input");
        selAllCb.type = "checkbox";
        selAllRow.append(selAllCb, document.createTextNode("Select All"));
        accPicker.appendChild(selAllRow);

        items.forEach(item => {
          const row = document.createElement("label");
          row.style.cssText = "display:flex;align-items:center;gap:6px;padding:2px 4px;font-size:10px;color:var(--text-secondary);cursor:pointer;border-radius:4px;";
          row.addEventListener("mouseenter", () => { row.style.background = "var(--bg-surface)"; });
          row.addEventListener("mouseleave", () => { row.style.background = ""; });
          const cb = document.createElement("input");
          cb.type = "checkbox";
          cb.dataset.id = item.id;
          cb.className = "acc-item-cb";
          const badge = document.createElement("span");
          badge.style.cssText = "font-size:8px;font-weight:700;text-transform:uppercase;color:var(--text-muted);";
          badge.textContent = item.type;
          const title = document.createElement("span");
          title.style.cssText = "flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;";
          title.textContent = item.title;
          row.append(cb, badge, title);
          accPicker.appendChild(row);
        });

        // Select all toggle
        selAllCb.addEventListener("change", () => {
          accPicker.querySelectorAll(".acc-item-cb").forEach(cb => { cb.checked = selAllCb.checked; });
        });

        accPicker.style.display = "flex";
        accConfirm.style.display = "";
      });

      accConfirm.addEventListener("click", async () => {
        const checkedIds = [...accPicker.querySelectorAll(".acc-item-cb:checked")].map(cb => cb.dataset.id);
        if (!checkedIds.length) return;

        const items = await AssetLibrary.list();
        const selected = items.filter(i => checkedIds.includes(i.id));

        // Build context text from selected assets
        const lines = ["[Asset Library Context]"];
        selected.forEach(item => {
          lines.push(`- [${item.type}] ${item.title}${item.description ? ': ' + item.description : ''}`);
          if (item.metadata) {
            const m = item.metadata;
            if (m.lat) lines.push(`  Location: ${m.lat.toFixed(6)}, ${m.lon.toFixed(6)}`);
            if (m.src) lines.push(`  URL: ${m.src}`);
            if (m.searchQuery) lines.push(`  Search: ${m.searchQuery}`);
            if (m.sceneA) lines.push(`  Scene: ${m.sceneA.satellite} ${m.sceneA.date} ${m.sceneA.time} cloud:${m.sceneA.cloudCover?.toFixed(0)}%`);
          }
        });

        // Append to existing context
        const contextAddition = lines.join("\n");
        opts.contextData = (opts.contextData || "") + "\n\n" + contextAddition;
        conversationId = null; // force re-seed so AI gets the new context

        // Visual feedback
        accConfirm.textContent = `Added ${selected.length}!`;
        setTimeout(() => {
          accConfirm.textContent = "Add to Context";
          accPicker.style.display = "none";
          accConfirm.style.display = "none";
      ...
      accRow.append(accBtn, accPicker, accConfirm);
      accRow.style.display = "flex";
      accRow.style.flexWrap = "wrap";
      accRow.style.alignItems = "flex-start";
      accRow.style.gap = "4px";
    }

    panel.append(messagesEl, accRow, inputRow);
