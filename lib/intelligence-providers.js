...
const IntelProviders = (() => {
  ...
  const PROVIDER_META = {
    ...
    dol:           { label: "DOL (MSHA)",     domain: "finance",    zeroConfig: false },
  };
...
function evaluatePixel(s){var v=(s.B8A-s.B11)/(s.B8A+s.B11);return v<0?[0.8,0.2,0]:v<0.3?[1,1,0]:[0,0.3+v*0.7,1]}`,
  ...
  // Department of Labor v4 API. Currently MSHA (mine safety) datasets.
  // OSHA not yet in v4 — will be added when DOL migrates it.
  ...
  const dol = {
    ...
    async searchMines(query, options = {}) {
      ...
      const limit = options.limit || 25;
      const filterObj = JSON.stringify({ field: "mine_name", operator: "like", value: `%${query}%` });
      const url = `${DOL_BASE}/get/MSHA/address_of_records_mines/json?X-API-KEY=${encodeURIComponent(key)}&limit=${limit}&filter_object=${encodeURIComponent(filterObj)}`;
      const resp = await fetch(url);
      if (!resp.ok) throw new Error(`DOL MSHA Mines: ${resp.status}`);
      const text = await resp.text();
    ...
    async searchAccidents(query, options = {}) {
      ...
      const limit = options.limit || 25;
      const filterObj = JSON.stringify({ field: "mine_name", operator: "like", value: `%${query}%` });
      const url = `${DOL_BASE}/get/MSHA/accident/json?X-API-KEY=${encodeURIComponent(key)}&limit=${limit}&filter_object=${encodeURIComponent(filterObj)}`;
      const resp = await fetch(url);
      if (!resp.ok) throw new Error(`DOL MSHA Accidents: ${resp.status}`);
      const text = await resp.text();
      if (text.includes("API key is either incorrect")) throw new Error("DOL API key is invalid — configure in Settings → Providers → DOL");
      const json = JSON.parse(text);
      const results = Array.isArray(json) ? json : (json.data || json.results || []);
    ...
    async search(query, options = {}) {
      // Fan out to both MSHA datasets
      const [mines, accidents] = await Promise.allSettled([
        this.searchMines(query, options),
        this.searchAccidents(query, options),
      ]);
      const results = [];
      if (mines.status === "fulfilled" && mines.value.results) {
        mines.value.results.forEach(r => { r._dolDataset = "mines"; results.push(r); });
      }
      if (accidents.status === "fulfilled" && accidents.value.results) {
        accidents.value.results.forEach(r => { r._dolDataset = "accidents"; results.push(r); });
      }
      return { total: results.length, results: results };
    },
  };
