...
const IntelProviders = (() => {
  ...
  const sentinelhub = {
    ...
    async searchCatalog(bbox, dateFrom, dateTo, options = {}) {
      const token = await this._getToken();
      const maxCC = options.maxCloudCoverage || 100; // default: show all, let user pick
      ...
      // Parse STAC features into a clean list
      return (data.features || []).map(f => {
        const dt = new Date(f.properties.datetime);
        // Build tight ±30s window around exact acquisition time
        ...
        return {
          ...
          bbox: f.bbox,
          // Tight window for getImage() — guarantees this exact scene
