...
const IntelProviders = (() => {
  ...
  const secedgar = {
    ...
    async searchCompany(name, options = {}) {
      let url = `https://efts.sec.gov/LATEST/search-index?q=%22${encodeURIComponent(name)}%22`;
      if (options.dateFrom || options.dateTo) {
        url += `&dateRange=custom`;
        if (options.dateFrom) url += `&startdt=${encodeURIComponent(options.dateFrom)}`;
        if (options.dateTo) url += `&enddt=${encodeURIComponent(options.dateTo)}`;
      }
...
function evaluatePixel(s){var v=(s.B8A-s.B11)/(s.B8A+s.B11);return v<0?[0.8,0.2,0]:v<0.3?[1,1,0]:[0,0.3+v*0.7,1]}`,
  ...
  const usaspending = {
    ...
    async search(query, options = {}) {
      ...
      // Date filtering via time_period
      if (options.dateFrom || options.dateTo) {
        filters.time_period = [{
          start_date: options.dateFrom || "2000-01-01",
          end_date: options.dateTo || new Date().toISOString().slice(0, 10),
        }];
      }
      const body = {
        filters: filters,
  ...
  /** Inject date range into a DOL filter object */
  function _dolAddDateFilter(filterObj, dateField, dateFrom, dateTo) {
    if (!dateFrom && !dateTo) return filterObj;
    var dateConditions = [];
    if (dateFrom) dateConditions.push({ field: dateField, operator: "gt", value: dateFrom });
    if (dateTo) dateConditions.push({ field: dateField, operator: "lt", value: dateTo });
    // Wrap existing filter + date conditions in "and"
    var existing = filterObj.and ? filterObj.and : [filterObj];
    return { "and": existing.concat(dateConditions) };
  }
  ...
  const dol = {
    ...
    async searchEBSA(query, options = {}) {
      ...
      var filter = { field: "plan_admin", operator: "like", value: `%${q}%` };
      filter = _dolAddDateFilter(filter, "final_close_date", options.dateFrom, options.dateTo);
    ...
    async _searchWHDFiltered(query, actField, label, options = {}) {
      ...
      var filter = actField
        ...
        : nameFilter;
      filter = _dolAddDateFilter(filter, "findings_start_date", options.dateFrom, options.dateTo);
    ...
    async searchOSHAInspections(query, options = {}) {
      ...
      const limit = options.limit || 25;
      const q = query.toUpperCase();
      var filter = { field: "estab_name", operator: "like", value: `%${q}%` };
      filter = _dolAddDateFilter(filter, "open_date", options.dateFrom, options.dateTo);
    ...
    async searchOSHAAccidents(query, options = {}) {
      ...
      const q = query.toUpperCase();
      var filter = { field: "estab_name", operator: "like", value: `%${q}%` };
      filter = _dolAddDateFilter(filter, "event_date", options.dateFrom, options.dateTo);
    ...
    async searchAccidents(query, options = {}) {
      ...
      const limit = options.limit || 25;
      const q = query.toUpperCase();
      var filter = { field: "operator_name", operator: "like", value: `%${q}%` };
      filter = _dolAddDateFilter(filter, "accident_date", options.dateFrom, options.dateTo);
    ...
    async searchMSHAViolations(query, options = {}) {
      ...
      const q = query.toUpperCase();
      var filter = { field: "operator_name", operator: "like", value: `%${q}%` };
      filter = _dolAddDateFilter(filter, "violation_issue_dt", options.dateFrom, options.dateTo);
