...
const ArgusDB = (() => {
  ...
  const PageTracker = {
    ...
    async setTrawlData(url, trawlData) {
      let entry = await this.getByUrl(url);
      if (!entry) return null;
      if (entry.trawlData) {
        const merge = (a, b) => [...new Set([...(a || []), ...(b || [])])];
        entry.trawlData.emails = merge(entry.trawlData.emails, trawlData.emails);
        entry.trawlData.phones = merge(entry.trawlData.phones, trawlData.phones);
        entry.trawlData.addresses = merge(entry.trawlData.addresses, trawlData.addresses);
        entry.trawlData.socialLinks = merge(entry.trawlData.socialLinks, trawlData.socialLinks);
        if (trawlData.businessName) entry.trawlData.businessName = trawlData.businessName;
        if (trawlData.geo) entry.trawlData.geo = trawlData.geo;
        if (trawlData.schema) entry.trawlData.schema = trawlData.schema;
        if (trawlData.meta) entry.trawlData.meta = trawlData.meta;
        if (trawlData.contacts?.length) {
          const seen = new Set((entry.trawlData.contacts || []).map(c => c.email));
          for (const c of trawlData.contacts) {
            if (c.email && !seen.has(c.email)) { entry.trawlData.contacts.push(c); seen.add(c.email); }
          }
        }
        entry.trawlData.scrollDepth = Math.max(entry.trawlData.scrollDepth || 0, trawlData.scrollDepth || 0);
        entry.trawlData.extractedAt = trawlData.extractedAt;
      } else {
        entry.trawlData = trawlData;
      }
      await put("pageTracker", entry);
      return entry;
    },
    async addDwellTime(url, ms) {
      let entry = await this.getByUrl(url);
      if (!entry || !entry.trawlData) return null;
      entry.trawlData.dwellTimeMs = (entry.trawlData.dwellTimeMs || 0) + ms;
      await put("pageTracker", entry);
      return entry;
    },
  };
