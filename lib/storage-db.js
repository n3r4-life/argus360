...
const ArgusDB = (() => {
  ...
  const STORES = {
    ...
    bookmarks:    { keyPath: "id", indexes: [{ name: "url", keyPath: "url" }, { name: "savedAt", keyPath: "savedAt" }] },
  ...
  const DEFAULT_PRUNE_RULES = [
    // History: keep maxHistorySize entries, compress after 30 days
    { store: "history", maxCount: 500, indexName: "timestamp", compress: true, compressAge: 30 * 24 * 60 * 60 * 1000 },
    // Snapshots: keep last 50 per monitor
    { store: "snapshots", groupBy: "monitorId", maxPerGroup: 50, indexName: "capturedAt" },
    // Changes: keep last 100 per monitor
    { store: "changes", groupBy: "monitorId", maxPerGroup: 100, indexName: "detectedAt" },
  ...
  const Snapshots = {
    ...
    async pruneForMonitor(monitorId, keepCount = 50) {
  ...
  return {
    ...
    estimateSize,
    // High-level stores
    History, Bookmarks, Projects, Monitors, Snapshots, Changes, Feeds, FeedEntries, Watchlist,
    KGNodes, KGEdges,
  };
})();
